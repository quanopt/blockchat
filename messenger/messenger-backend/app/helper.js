...
function newRemotes(urls, forPeers, userOrg) {
	...
	for (let index in urls) {
		...
		if (!found) {
			console.log(`Failed to find a peer matching the url ${peerUrl}`);
		}
	}
...
var getRegisteredUsers = function(username, userOrg, isJson) {
	...
	return hfc.newDefaultKeyValueStore({
		...
		return client.getUserContext(username, true).then((user) => {
			if (user && user.isEnrolled()) {
				...
				return getAdminUser(userOrg).then(function(adminUserObj) {
					...
					console.log(`${username} enroll failed: ${err.stack ? err.stack : err}`);
					return '' + err;
				});;
			}
		});
		...
		console.log(`Failed to get registered user: ${username}, error: ${err.stack ? err.stack : err}`);
		return '' + err;
	});
};
